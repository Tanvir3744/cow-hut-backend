...
const createCow = async (payload: ICow): Promise<ICow | null> => {
  ...
  try {
    ...
    const createCow = await Cows.create(payload, { session });
    if (!createCow.length) {
  ...
  if (newCowData) {
    newCowData = await Cows.findOne(newCowData._id).populate({
      path: "seller",
    });
  }
  console.log(newCowData?._id, "cow id")
  return newCowData;
}

const getSingleCow = async (id: string,): Promise<ICow | null> => {
  const result = await Cows.findById(id).populate
  if (!result) {
