...
const createSeller = async (user: IUser, seller: ISeller) => {
  const { name, address, budget, income, phoneNumber } = seller;
  ...
  try {
    ...
    const newSeller = await Seller.create([{{firstName, lastName},  address, budget, income, phoneNumber}], { session });

    if (!seller.length) {
      throw new ApiError(status.BAD_REQUEST, 'Failed to createSeller');
    }

    user.seller = seller[0]._id;
